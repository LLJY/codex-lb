...
def _install_single_account_bridge_upstreams(
    monkeypatch: pytest.MonkeyPatch,
    *,
    account: Account,
    upstreams: list[Any],
...
class _PrecreatedCancelThenLeakUpstreamWebSocket(_FakeBridgeUpstreamWebSocket):
    def __init__(self) -> None:
        super().__init__()
        self.first_request_submitted = asyncio.Event()

    async def send_text(self, text: str) -> None:
        self.sent_text.append(text)
        if len(self.sent_text) == 1:
            self.first_request_submitted.set()
            return
...
class _ResponseLessReplayUpstreamWebSocket(_FakeBridgeUpstreamWebSocket):
    async def send_text(self, text: str) -> None:
        self.sent_text.append(text)
        response_id = "resp_response_less_replay"
        await self._messages.put(
            _FakeUpstreamMessage(
                "text",
                text=json.dumps(
...
class _CreatedDeltaThenCloseUpstreamWebSocket(_FakeBridgeUpstreamWebSocket):
    def __init__(self, before_close: Callable[[], Awaitable[None]] | None = None) -> None:
        super().__init__()
        self.before_close = before_close

    async def send_text(self, text: str) -> None:
        self.sent_text.append(text)
        response_id = f"resp_created_delta_close_{len(self.sent_text)}"
        await self._messages.put(
            _FakeUpstreamMessage(
                "text",
                text=json.dumps(
    ...
    async def receive(self) -> _FakeUpstreamMessage:
        ...
        if message.kind != "text" and self.before_close is not None:
...
@pytest.mark.asyncio
async def test_restore_http_bridge_session_indexes_rejects_inflight_replacement(app_instance):
    service = get_proxy_service_for_app(app_instance)
    service._http_bridge_sessions.clear()
    service._http_bridge_turn_state_index.clear()
    service._http_bridge_inflight_sessions.clear()

    session_key = proxy_module._HTTPBridgeSessionKey("prompt_cache", "restore-inflight-key", None)
    recovered_session = cast(proxy_module._HTTPBridgeSession, _make_dummy_bridge_session(session_key))
    inflight_future: asyncio.Future = asyncio.get_running_loop().create_future()
    service._http_bridge_inflight_sessions[session_key] = inflight_future

    try:
        restored = await service._restore_http_bridge_session_indexes_if_available(recovered_session)
    finally:
        service._http_bridge_inflight_sessions.clear()
        inflight_future.cancel()

    assert restored is False
    assert session_key not in service._http_bridge_sessions


@pytest.mark.asyncio
async def test_restore_http_bridge_session_indexes_rejects_turn_state_alias_replacement(app_instance):
    service = get_proxy_service_for_app(app_instance)
    service._http_bridge_sessions.clear()
    service._http_bridge_turn_state_index.clear()
    service._http_bridge_inflight_sessions.clear()

    turn_state = "http_turn_restore_alias"
    session_key = proxy_module._HTTPBridgeSessionKey("prompt_cache", "restore-alias-key", None)
    alias_key = proxy_module._HTTPBridgeSessionKey("turn_state_header", turn_state, None)
    recovered_session = cast(proxy_module._HTTPBridgeSession, _make_dummy_bridge_session(session_key))
    recovered_session.downstream_turn_state_aliases.add(turn_state)
    inflight_future: asyncio.Future = asyncio.get_running_loop().create_future()
    service._http_bridge_inflight_sessions[alias_key] = inflight_future
    ...
    assert restored is False
    assert session_key not in service._http_bridge_sessions
    assert proxy_module._http_bridge_turn_state_alias_key(turn_state, None) not in service._http_bridge_turn_state_index

    replacement_session = cast(proxy_module._HTTPBridgeSession, _make_dummy_bridge_session(alias_key))
    service._http_bridge_sessions[alias_key] = replacement_session
    restored = await service._restore_http_bridge_session_indexes_if_available(recovered_session)

    assert restored is False
    assert service._http_bridge_sessions[alias_key] is replacement_session
    assert session_key not in service._http_bridge_sessions
    assert proxy_module._http_bridge_turn_state_alias_key(turn_state, None) not in service._http_bridge_turn_state_index
...
@pytest.mark.asyncio
async def test_v1_responses_http_bridge_retries_created_without_output_without_leaking_stale_id(
    async_client,
    monkeypatch,
):
    _install_bridge_settings(monkeypatch, enabled=True)
    account_id = await _import_account(
        async_client,
        "acc_http_bridge_created_retry",
        "http-bridge-created-retry@example.com",
    )
    account = await _get_account(account_id)
    first_upstream = _CreatedThenCloseUpstreamWebSocket()
    ...
    events = await _collect_sse_events(
        ...
        json_body={
            "model": "gpt-5.1",
            "instructions": "Return exactly OK.",
            "input": "retry-after-created",
            "prompt_cache_key": "created-retry-key",
            "stream": True,
        },
    )
...
async def test_v1_responses_http_bridge_created_retry_preserves_replay_response_less_events(
    ...
    account_id = await _import_account(
        async_client,
        "acc_http_bridge_created_retry_response_less",
        "http-bridge-created-retry-response-less@example.com",
    )
    account = await _get_account(account_id)
    first_upstream = _CreatedThenCloseUpstreamWebSocket()
    retry_upstream = _ResponseLessReplayUpstreamWebSocket()
    connect_count = _install_single_account_bridge_upstreams(
        monkeypatch,
        account=account,
        upstreams=[first_upstream, retry_upstream],
    ...
    events = await _collect_sse_events(
        ...
        json_body={
            "model": "gpt-5.1",
            "instructions": "Return exactly OK.",
            "input": "retry-after-created-response-less",
            "prompt_cache_key": "created-retry-response-less-key",
            "stream": True,
        },
    )
...
@pytest.mark.asyncio
async def test_v1_responses_http_bridge_does_not_retry_after_output_delta_reaches_downstream(
    async_client,
    app_instance,
    monkeypatch,
):
    ...
    account_id = await _import_account(
        async_client,
        "acc_http_bridge_created_delta_no_retry",
        "http-bridge-created-delta-no-retry@example.com",
    )
    account = await _get_account(account_id)
    service = get_proxy_service_for_app(app_instance)

    async def wait_for_delta_to_reach_downstream_state() -> None:
            ...
            while True:
                ...
                for session in sessions:
                    async with session.pending_lock:
                        if any(
                            request_state.response_created_downstream_flushed
                            and request_state.latency_first_token_ms is not None
                            for request_state in session.pending_requests
                        ):
                            return
                await asyncio.sleep(0)

        await asyncio.wait_for(wait_for_state(), timeout=_TEST_SYNC_TIMEOUT_SECONDS)

    upstream = _CreatedDeltaThenCloseUpstreamWebSocket(before_close=wait_for_delta_to_reach_downstream_state)
    connect_count = _install_single_account_bridge_upstreams(monkeypatch, account=account, upstreams=[upstream])
    ...
    events = await _collect_sse_events(
        ...
        json_body={
            "model": "gpt-5.1",
            "instructions": "Return exactly OK.",
            "input": "do-not-retry-after-delta",
            "prompt_cache_key": "created-delta-no-retry-key",
            "stream": True,
        },
    )
...
@pytest.mark.asyncio
async def test_detach_http_bridge_request_retires_session_before_close_completes(app_instance, monkeypatch):
    _install_bridge_settings(monkeypatch, enabled=True)
    service = get_proxy_service_for_app(app_instance)
    service._http_bridge_sessions.clear()
    service._http_bridge_inflight_sessions.clear()
    ...
    session_key = proxy_module._HTTPBridgeSessionKey("prompt_cache", "detach-race-key", None)
    affinity = proxy_module._AffinityPolicy(
        key="detach-race-key",
        kind=proxy_module.StickySessionKind.PROMPT_CACHE,
        max_age_seconds=300,
    )
    old_upstream = _RecordingUpstreamWebSocket()
    old_session = proxy_module._HTTPBridgeSession(
        key=session_key,
        headers={},
        affinity=affinity,
        request_model="gpt-5.1",
        account=cast(Account, SimpleNamespace(id="acct-detach-race", status=AccountStatus.ACTIVE)),
        upstream=cast(proxy_module.UpstreamResponsesWebSocket, old_upstream),
        upstream_control=proxy_module._WebSocketUpstreamControl(),
        pending_requests=deque(),
        pending_lock=anyio.Lock(),
        response_create_gate=asyncio.Semaphore(1),
    ...
    detached_request = proxy_module._WebSocketRequestState(
        request_id="req-detach-race-a",
        model="gpt-5.1",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
        started_at=time.monotonic(),
        response_id="resp-detach-race-a",
        event_queue=asyncio.Queue(),
        transport="http",
    )
    old_session.pending_requests.append(detached_request)
    service._http_bridge_sessions[session_key] = old_session

    close_started = asyncio.Event()
    allow_close_finish = asyncio.Event()
    create_calls = 0
    replacement_upstream = _RecordingUpstreamWebSocket()

    async def fake_close_http_bridge_session(self, target_session, *, turn_state_lock_held=False):
        del self, turn_state_lock_held
        target_session.closed = True
        if target_session is old_session:
            close_started.set()
            await allow_close_finish.wait()
    ...
    async def fake_create_http_bridge_session(
        ...
        *,
        ...
        del self, headers, affinity, api_key, request_model, idle_ttl_seconds
        ...
        return proxy_module._HTTPBridgeSession(
            ...
            request_model="gpt-5.1",
            account=cast(Account, SimpleNamespace(id="acct-detach-race", status=AccountStatus.ACTIVE)),
            upstream=cast(proxy_module.UpstreamResponsesWebSocket, replacement_upstream),
            upstream_control=proxy_module._WebSocketUpstreamControl(),
            pending_requests=deque(),
            pending_lock=anyio.Lock(),
            response_create_gate=asyncio.Semaphore(1),
    ...
    detach_task = asyncio.create_task(service._detach_http_bridge_request(old_session, request_state=detached_request))
    await asyncio.wait_for(close_started.wait(), timeout=1.0)

    new_session = await service._get_or_create_http_bridge_session(
        session_key,
        headers={},
        affinity=affinity,
        api_key=None,
    ...
    next_request = proxy_module._WebSocketRequestState(
        request_id="req-detach-race-b",
        model="gpt-5.1",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    next_text = json.dumps({"type": "response.create", "model": "gpt-5.1", "input": "next"})
    await service._submit_http_bridge_request(
        new_session, request_state=next_request, text_data=next_text, queue_limit=8
    )

    allow_close_finish.set()
    assert await detach_task is True

    assert create_calls == 1
    assert new_session is not old_session
    assert service._http_bridge_sessions[session_key] is new_session
    assert old_upstream.sent_text == []
    assert replacement_upstream.sent_text == [next_text]


@pytest.mark.asyncio
async def test_queued_http_bridge_request_does_not_send_after_session_retired(app_instance):
    service = get_proxy_service_for_app(app_instance)
    service._http_bridge_sessions.clear()
    service._http_bridge_inflight_sessions.clear()
    service._http_bridge_turn_state_index.clear()

    session_key = proxy_module._HTTPBridgeSessionKey("prompt_cache", "detach-queued-key", None)
    old_upstream = _RecordingUpstreamWebSocket()
    session = proxy_module._HTTPBridgeSession(
        key=session_key,
        headers={},
        affinity=proxy_module._AffinityPolicy(
            key="detach-queued-key",
            kind=proxy_module.StickySessionKind.PROMPT_CACHE,
            max_age_seconds=300,
        ),
        request_model="gpt-5.1",
        account=cast(Account, SimpleNamespace(id="acct-detach-queued", status=AccountStatus.ACTIVE)),
        upstream=cast(proxy_module.UpstreamResponsesWebSocket, old_upstream),
        upstream_control=proxy_module._WebSocketUpstreamControl(),
        pending_requests=deque(),
        pending_lock=anyio.Lock(),
    ...
    active_request = proxy_module._WebSocketRequestState(
        request_id="req-detach-queued-a",
        model="gpt-5.1",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
        started_at=time.monotonic(),
        response_id="resp-detach-queued-a",
        event_queue=asyncio.Queue(),
        transport="http",
        response_create_gate=session.response_create_gate,
        response_create_gate_acquired=True,
    )
    queued_request = proxy_module._WebSocketRequestState(
        request_id="req-detach-queued-b",
        model="gpt-5.1",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    session.pending_requests.append(active_request)
    service._http_bridge_sessions[session_key] = session

    submit_task = asyncio.create_task(
        service._submit_http_bridge_request(
            session,
            request_state=queued_request,
            text_data=json.dumps({"type": "response.create", "model": "gpt-5.1", "input": "next"}),
            queue_limit=8,
        )
    )
    for _ in range(100):
        ...
        raise AssertionError("queued request did not wait on bridge gate")

    assert await service._detach_http_bridge_request(session, request_state=active_request) is True
    with pytest.raises(proxy_module.ProxyResponseError) as exc_info:
...
@pytest.mark.asyncio
async def test_http_bridge_prewarm_does_not_send_after_session_retired(app_instance, monkeypatch):
    _install_bridge_settings_with_limits(monkeypatch, enabled=True, codex_prewarm_enabled=True)
    service = get_proxy_service_for_app(app_instance)
    old_upstream = _RecordingUpstreamWebSocket()
    session = proxy_module._HTTPBridgeSession(
        key=proxy_module._HTTPBridgeSessionKey("turn_state_header", "prewarm-retired-key", None),
        headers={},
        affinity=proxy_module._AffinityPolicy(
            key="prewarm-retired-key",
            kind=proxy_module.StickySessionKind.CODEX_SESSION,
            max_age_seconds=300,
        ),
        request_model="gpt-5.1",
        account=cast(Account, SimpleNamespace(id="acct-prewarm-retired", status=AccountStatus.ACTIVE)),
        upstream=cast(proxy_module.UpstreamResponsesWebSocket, old_upstream),
        upstream_control=proxy_module._WebSocketUpstreamControl(),
        pending_requests=deque(),
        pending_lock=anyio.Lock(),
    ...
    request_state = proxy_module._WebSocketRequestState(
        request_id="req-prewarm-retired",
        model="gpt-5.1",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    async def fake_acquire_response_create_admission(request_state, *, response_create_gate, compact=False):
        del compact
    ...
    monkeypatch.setattr(
        ...
        fake_acquire_response_create_admission,
    )
    ...
    assert exc_info.value.payload["error"]["code"] == "upstream_unavailable"
    assert list(session.pending_requests) == []
    assert session.queued_request_count == 3
    assert old_upstream.sent_text == []


@pytest.mark.asyncio
async def test_close_http_bridge_session_can_skip_stale_durable_release(app_instance, monkeypatch):
    service = get_proxy_service_for_app(app_instance)
    release_live_session = AsyncMock()
    monkeypatch.setattr(service, "_durable_bridge", SimpleNamespace(release_live_session=release_live_session))
    session = proxy_module._HTTPBridgeSession(
        key=proxy_module._HTTPBridgeSessionKey("prompt_cache", "skip-release-key", None),
        headers={},
        affinity=proxy_module._AffinityPolicy(
            key="skip-release-key",
            kind=proxy_module.StickySessionKind.PROMPT_CACHE,
            max_age_seconds=300,
        ),
        request_model="gpt-5.1",
        account=cast(Account, SimpleNamespace(id="acct-skip-release", status=AccountStatus.ACTIVE)),
        upstream=cast(proxy_module.UpstreamResponsesWebSocket, _RecordingUpstreamWebSocket()),
        upstream_control=proxy_module._WebSocketUpstreamControl(),
        pending_requests=deque(),
        pending_lock=anyio.Lock(),
        response_create_gate=asyncio.Semaphore(1),
        ...
        last_used_at=time.monotonic(),
        idle_ttl_seconds=120.0,
        durable_session_id="durable-skip-release",
        durable_owner_epoch=7,
        release_durable_on_close=False,
    )
...
@pytest.mark.asyncio
async def test_close_http_bridge_session_closes_captured_upstream_after_session_mutates(app_instance, monkeypatch):
    service = get_proxy_service_for_app(app_instance)
    old_upstream = _RecordingUpstreamWebSocket()
    new_upstream = _RecordingUpstreamWebSocket()
    old_reader = asyncio.create_task(asyncio.sleep(3600))
    new_reader = asyncio.create_task(asyncio.sleep(3600))
    session = proxy_module._HTTPBridgeSession(
        key=proxy_module._HTTPBridgeSessionKey("prompt_cache", "close-snapshot-key", None),
        headers={},
        affinity=proxy_module._AffinityPolicy(
            key="close-snapshot-key",
            kind=proxy_module.StickySessionKind.PROMPT_CACHE,
            max_age_seconds=300,
        ),
        request_model="gpt-5.1",
        account=cast(Account, SimpleNamespace(id="acct-close-snapshot", status=AccountStatus.ACTIVE)),
        upstream=cast(proxy_module.UpstreamResponsesWebSocket, old_upstream),
        upstream_control=proxy_module._WebSocketUpstreamControl(),
        pending_requests=deque(),
        pending_lock=anyio.Lock(),
    ...
    session.upstream = cast(proxy_module.UpstreamResponsesWebSocket, new_upstream)
    session.upstream_reader = new_reader

    allow_cancel_finish.set()
    await close_task

    assert old_upstream.closed is True
    assert new_upstream.closed is False
    assert session.upstream_reader is new_reader
...
async def test_v1_responses_http_bridge_ignores_stale_top_level_response_id_after_cancel(
    ...
    service = get_proxy_service_for_app(app_instance)
    account = await _get_account(account_id)
    upstreams = [_AbandonedRequestLeakUpstreamWebSocket(), _FakeBridgeUpstreamWebSocket()]
    connect_count = 0
...
@pytest.mark.asyncio
async def test_v1_responses_http_bridge_reconnects_after_cancel_before_response_created(
    async_client,
    app_instance,
    monkeypatch,
):
    ...
    account_id = await _import_account(
        async_client,
        "acc_http_bridge_precreated_cancel",
        "http-bridge-precreated-cancel@example.com",
    )
    service = get_proxy_service_for_app(app_instance)
    account = await _get_account(account_id)
    first_upstream = _PrecreatedCancelThenLeakUpstreamWebSocket()
    ...
    first_payload = proxy_module.ResponsesRequest(
        model="gpt-5.1",
        instructions="Return exactly OK.",
        input="cancel-before-created",
        prompt_cache_key="precreated-cancel-key",
    )
    first_stream = cast(
        AsyncGenerator[str, None],
        service._stream_via_http_bridge(
    ...
    await asyncio.wait_for(first_upstream.first_request_submitted.wait(), timeout=1.0)
    first_task.cancel()
